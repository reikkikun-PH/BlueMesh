...
- **Proximity Distance Detection**: Detect other users' approximate distance via Bluetooth signal strength (RSSI) using the Log-Distance Path Loss model (only shown if the detected user has opted-in to share location).
- **Opt-In Location Privacy**: A security setting ("Share Device Location" - "Allow to share proximity location") that lets users decide whether to share their location. By default, a user cannot be detected by proximity distance estimators unless they explicitly enable this option.
